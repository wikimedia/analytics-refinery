...
CREATE EXTERNAL TABLE IF NOT EXISTS `webrequest`(
    ...
    `is_redirect_to_pageview` boolean COMMENT 'Indicates if this record was marked as a redirect to a pageview during refinement. This field was introduced in 2024-11, it is always NULL before that.',
    `is_api_request`          boolean COMMENT 'Indicates if this record was classified as a request to Wikimedia APIs',
    ...
    `pageview_info`           map<string, string>  COMMENT 'map containing project, language_variant and page_title values only when is_pageview = TRUE or is_redirect_to_pageview = TRUE (the latter only after 2024-11).',
