-- Backfills the monthly per-domain unique devices from the original hive table.
--
...
--         -f backfill_unique_devices_per_domain_monthly_iceberg.hql \
--         -d unique_devices_hive_table=wmf.unique_devices_per_domain_monthly \
--         -d unique_devices_iceberg_table=wmf_readership.unique_devices_per_domain_monthly
...
GROUP BY
    ...
    country,
    country_code,
    TO_DATE(CONCAT_WS('-', LPAD(year, 4, '0'), LPAD(month, 2, '0'), '01'), 'yyyy-MM-dd')
