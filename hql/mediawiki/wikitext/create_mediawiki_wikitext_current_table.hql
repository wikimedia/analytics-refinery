-- Creates table statement for mediawiki_wikitext_current table.
--
...
--
-- Usage
--     hive -f create_mediawiki_wikitext_current_table.hql \
--         --database wmf
--


CREATE EXTERNAL TABLE `mediawiki_wikitext_current`(
  ...
  `user_id`                                       bigint        COMMENT 'id of the user that made the revision (or null/0 if anonymous)',
...
COMMENT
  'See most up to date documentation at https://wikitech.wikimedia.org/wiki/Analytics/Data_Lake/Mediawiki_wikitext_current'
PARTITIONED BY (
  `snapshot` string COMMENT 'Versioning information to keep multiple datasets (YYYY-MM for regular imports)',
  `wiki_db` string COMMENT 'The wiki_db project')
STORED AS AVRO
LOCATION
  'hdfs://analytics-hadoop/wmf/data/wmf/mediawiki/wikitext/current'
;
