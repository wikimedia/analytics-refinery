...
CREATE EXTERNAL TABLE IF NOT EXISTS `mediawiki_history_incremental_v1` (
    ...
    `row_update_dt`                                                    timestamp     COMMENT 'Incremental-read watermark of the run that last wrote this row. Daily (source=events) writes use the run data_interval_end, a UTC midnight boundary, advanced via GREATEST so back-patches and out-of-order reruns never lower it. The monthly reconcile (source=snapshot) uses the last second of the snapshot month, i.e. exactly one second below the next month-boundary midnight; the daily pipeline blocks on the 1st until the reconcile lands, so it orders after the dailies of that month and before the dailies of the next as a single monotonic event. That 1-second gap is the opt-in/opt-out switch: a lower bound of WHERE row_update_dt >= a month-boundary midnight returns daily deltas only and skips the reconcile, while lowering that same bound by INTERVAL 1 SECOND also pulls the monthly re-baseline (the full snapshot, billions of rows). Pair with an event_timestamp filter for partition pruning.'
)
USING ICEBERG
TBLPROPERTIES (
