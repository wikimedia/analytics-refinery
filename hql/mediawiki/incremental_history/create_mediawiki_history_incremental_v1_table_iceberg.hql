...
CREATE EXTERNAL TABLE IF NOT EXISTS `mediawiki_history_incremental_v1` (
    ...
    `row_update_dt`                                                    timestamp     COMMENT 'Incremental-read watermark: the Airflow data_interval_end (run day + 1, UTC) of the daily run that last wrote this row. Advanced via GREATEST so back-patches and out-of-order reruns never lower it. Consumers read deltas via WHERE row_update_dt >= last_watermark (pair with an event_timestamp filter for partition pruning). NULL for source=snapshot rows: the monthly reconcile is a re-baseline, picked up via full reload rather than the watermark.'
)
USING ICEBERG
TBLPROPERTIES (
