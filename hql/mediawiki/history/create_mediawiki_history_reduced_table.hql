...
CREATE EXTERNAL TABLE IF NOT EXISTS `mediawiki_history_reduced` (
  ...
  `user_type`            string COMMENT 'anonymous, group_bot, name_bot or user. Temporary users are labeled as anonymous for backwards compatibility.',
  ...
  `user_id`              bigint COMMENT 'User ID (local to the project/wiki)',
  `user_central_id`      bigint COMMENT 'User central (or global) ID',
