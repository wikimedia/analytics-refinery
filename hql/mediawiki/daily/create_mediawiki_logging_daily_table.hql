-- Creates table statement for raw mediawiki_logging_daily table.
--
-- Parameters:
--     <none>
--
-- Usage
--     hive -f create_mediawiki_logging_daily_table.hql \
--         --database wmf_raw
--

CREATE EXTERNAL TABLE `mediawiki_logging_daily`(
...
PARTITIONED BY (
  `day` string COMMENT 'The day in YYYY-MM-DD format, all logging events from that day are imported',
...
LOCATION
  'hdfs://analytics-hadoop/wmf/data/raw/mediawiki/tables/daily/logging'
;
