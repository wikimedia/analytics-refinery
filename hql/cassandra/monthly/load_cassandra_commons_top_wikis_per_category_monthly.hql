-- Load commons.top_wikis_per_category_monthly dataset to cassandra.
--
-- Parameters:
...
--     -f load_cassandra_commons_top_wikis_per_category_monthly.hql \
--     -d destination_table=aqs.commons.top_wikis_per_category_monthly \
--     -d source_table=wmf_contributors.commons_pageviews_per_category_monthly  \
--     -d coalesce_partitions=3 \
--     -d year_month=2023-11

WITH total_pageviews_by_wiki AS (
    SELECT
        category,
        category_scope,
        wiki,
...
top_ranked_total_pageviews_by_wiki AS (
    SELECT *
    FROM (
        SELECT
            category,
...
top_ranked AS (
    ...
    FROM top_ranked_total_pageviews_by_wiki
)
