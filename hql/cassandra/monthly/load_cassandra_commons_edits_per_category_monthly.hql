-- Load commons.edits_per_category_monthly dataset to cassandra.
--
-- Parameters:
...
--     -f load_cassandra_commons_edits_per_category_monthly.hql \
--     -d destination_table=aqs.commons.edits_per_category_monthly \
...
prepared_edits_by_category AS (
     ...
     FROM primary_category_edits
     GROUP BY CUBE (category, edit_type)
...
/*+ COALESCE(${coalesce_partitions}) */
    ...
    edit_count,
    dt
FROM prepared_edits_by_category
WHERE category IS NOT NULL -- remove 'all-categories' nulls generated by GROUP BY CUBE, as they are not aggregatable
ORDER BY
    category,
    category_scope,
    edit_type,
