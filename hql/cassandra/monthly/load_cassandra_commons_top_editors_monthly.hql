-- Load commons.top_editors_monthly dataset to cassandra.
--
-- Parameters:
...
--     -f load_cassandra_commons_top_editors_monthly.hql \
--     -d destination_table=aqs.commons.top_editors_monthly \
...
exploded_categories AS (
    ...
    FROM (
         SELECT
             ...
             edit_count
         FROM total_edits_per_editor)
    GROUP BY CUBE(user_name, edit_type, category)
    -- remove 'all-categories' and 'all-user-names' nulls as they are not aggregatable
    HAVING user_name IS NOT NULL AND category IS NOT NULL
),
...
top_ranked_shallow_edits_per_category_per_edit_type AS (
    ...
    FROM (
        SELECT
            ...
            edit_count,
            RANK()       over (PARTITION BY category, edit_type ORDER BY edit_count DESC) AS rank,
            ROW_NUMBER() over (PARTITION BY category, edit_type ORDER BY edit_count DESC) AS row_number
