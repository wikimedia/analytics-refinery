-- Load commons.edits_per_user_monthly dataset to cassandra.
--
-- Parameters:
...
--     -f load_cassandra_commons_edits_per_user_monthly.hql \
--     -d destination_table=aqs.commons.edits_per_user_monthly \
...
prepared_edits_by_user AS (
     ...
     GROUP BY CUBE (user_name, edit_type)
)

INSERT INTO ${destination_table}
SELECT
/*+ COALESCE(${coalesce_partitions}) */
    COALESCE(user_name, ':all-user-names:') as user_name,  -- GROUP BY CUBE will set value to null when it is all-user-names
                                                           -- we use colons as they are not valid for user names thus avoid collisions
    COALESCE(edit_type, 'all-edit-types') as edit_type,    -- GROUP BY CUBE will set value to null when it is all-edit-types
    edit_count,
    dt
FROM prepared_edits_by_user
ORDER BY
    user_name,
    edit_type,
    dt
