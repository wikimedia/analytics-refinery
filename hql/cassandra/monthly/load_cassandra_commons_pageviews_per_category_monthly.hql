-- Load commons.pageviews_per_category_monthly dataset to cassandra.
--
-- Parameters:
...
--     -f load_cassandra_commons_pageviews_per_category_monthly.hql \
--     -d destination_table=aqs.commons.pageviews_per_category_monthly \
--     -d source_table=wmf_contributors.commons_pageviews_per_category_monthly  \
--     -d coalesce_partitions=3 \
--     -d year_month=2023-11

WITH prepared_pageviews_by_category AS (
    SELECT
        category,
        FIRST(category_scope) as category_scope,
        wiki,
        SUM(pageview_count) as pageview_count,
        FIRST(TO_TIMESTAMP(year_month)) as dt
    FROM ${source_table}
    WHERE year_month = '${year_month}' AND category_scope = 'shallow'
    GROUP BY CUBE (category, wiki)
    ...
    SELECT
        ...
        FIRST(TO_TIMESTAMP(year_month)) as dt
    FROM ${source_table}
    WHERE year_month = '${year_month}' AND category_scope = 'deep'
    GROUP BY CUBE (category, wiki)
)
...
/*+ COALESCE(${coalesce_partitions}) */
    COALESCE(category, 'all-categories') as category, -- GROUP BY CUBE will set value to null when it is all-categories
    category_scope,
    COALESCE(wiki, 'all-wikis') as wiki,              -- GROUP BY CUBE will set value to null when it is all-wikis
    pageview_count,
    dt
FROM prepared_pageviews_by_category
ORDER BY
    category,
    category_scope,
    wiki,
