-- Extracts one hour of json formatted hourly pageviews to be loaded in Druid
--
-- Usage:
--     hive -f generate_hourly_druid_pageviews.hql \
--         -d source_table=wmf.pageview_hourly \
--         -d destination_directory=/tmp/druid/hourly_json_pageviews \
--         -d year=2016 \
--         -d month=7 \
--         -d day=10 \
--         -d hour=10
...
DROP TABLE IF EXISTS tmp_hourly_druid_pageviews_${year}_${month}_${day}_${hour};


CREATE EXTERNAL TABLE IF NOT EXISTS tmp_hourly_druid_pageviews_${year}_${month}_${day}_${hour} (
    `dt`                  string,
    `project`             string,
...
INSERT OVERWRITE TABLE tmp_hourly_druid_pageviews_${year}_${month}_${day}_${hour}
SELECT
    CONCAT(
        LPAD(year, 4, '0'), '-',
        LPAD(month, 2, '0'), '-',
