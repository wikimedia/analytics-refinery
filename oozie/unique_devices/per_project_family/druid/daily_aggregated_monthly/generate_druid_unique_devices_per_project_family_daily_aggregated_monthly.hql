...
--
-- Usage:
--     hive -f generate_druid_unique_devices_per_project_family_daily_aggregsated_monthly.hql \
...
DROP TABLE IF EXISTS tmp_druid_unique_devices_per_project_family_daily_aggregated_monthly${year}_${month};


CREATE EXTERNAL TABLE IF NOT EXISTS tmp_druid_unique_devices_per_project_family_daily_aggregated_monthly${year}_${month} (
    `dt`                     string,
...
INSERT OVERWRITE TABLE tmp_druid_unique_devices_per_project_family_daily_aggregated_monthly${year}_${month}
SELECT
    CONCAT(
        LPAD(year, 4, '0'), '-',
        LPAD(month, 2, '0'), '-',
