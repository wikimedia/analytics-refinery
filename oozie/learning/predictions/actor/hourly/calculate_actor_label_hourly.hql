...
WITH actor_aggregated AS (
    SELECT
        ...
        CASE
            ...
            WHEN pageview_ratio_per_min >= 30 then 'automated - high request ratio'

            -- more than 10 requests without cookies, this is a problem in quite old phones and app versions
            -- that due to bugs send bursty request traffic. See: https://phabricator.wikimedia.org/T229068
            WHEN nocookies > 10 then 'automated - too many requests w/o cookies'

            WHEN user_agent_length > 400 or user_agent_length < 25 then 'automated - suspicious UA'
            -- low pageview ratios as low as 1 per min
            WHEN pageview_ratio_per_min = 0 then 'user - low request ratio'
