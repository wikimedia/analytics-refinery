-- Calculate the rollup of features per actor for the last 24 hours

-- Code calculates (per actor) the values for features for the 24 hours prior. While
-- we run our classification per hour, the features used to provide a label use
...
-- Usage:
-- hive -f calculate_actor_rollup.hql
--          -d source_table=features.actor_hourly
--          -d destination_table=features.actor_rollup_hourly
--          -d version=0.1
--          -d interval_end_year=2015
--          -d interval_end_month=6
--          -d interval_end_day=1
--          -d interval_end_hour=1
--          -d interval_start_year=2015
--          -d interval_start_month=5
--          -d interval_start_day=31
--          -d interval_start_hour=1
