-- Extracts one day of json formatted hourly virtualpageviews to be loaded in Druid
--
-- Usage:
...
DROP TABLE IF EXISTS tmp_daily_druid_virtualpageviews_${year}_${month}_${day};

CREATE EXTERNAL TABLE IF NOT EXISTS tmp_daily_druid_virtualpageviews_${year}_${month}_${day} (
    `dt`                  string,
    `project`             string,
...
INSERT OVERWRITE TABLE tmp_daily_druid_virtualpageviews_${year}_${month}_${day}
SELECT
    CONCAT(
        LPAD(year, 4, '0'), '-',
        LPAD(month, 2, '0'), '-',
