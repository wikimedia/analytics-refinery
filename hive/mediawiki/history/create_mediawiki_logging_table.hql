...
CREATE EXTERNAL TABLE `mediawiki_logging`(
  ...
  `log_user`        string      COMMENT 'The id of the user who performed the action. This is a reference into the user table (the user id of "Jacksprat" in example). Comparable to rc_user and rev_user (string because of decimal field being loaded as such by sqoop - should becast to long).',
