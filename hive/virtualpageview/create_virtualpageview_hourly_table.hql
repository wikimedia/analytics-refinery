...
CREATE EXTERNAL TABLE IF NOT EXISTS `virtualpageview_hourly`(
    ...
    `page_title`        string  COMMENT 'Page Title from EventLogging event (canonical)',
    ...
    `page_id`           bigint  COMMENT 'MediaWiki page_id',
    `namespace_id`      int     COMMENT 'MediaWiki namespace_id'
)
PARTITIONED BY (
